...
<html>
	...
	<body>
		<p class="path">当前位置:/电器进销存系统/销售管理/销售下单</p>
		<table border="0" cellspacing="0" cellpadding="0" class="tb tblist" id="cartable">
			...
			<tr data-proid="<%=m.get("proid")%>"><td><img src="upload/<%=m.get("imgurl")%>" style="width:120px;height:38px;padding:5px 0; "></td><td><%=m.get("proname")%></td><td><span><%=m.get("price")%></span>元</td><td><div class="pronumber"><span class="left">-</span><input type="text" value="<%=m.get("procount")%>"/><span class="right">+</span> </div>  </td><td><span></span>元</td><td><span class="del">删除</span></td></tr>
			<%}%>
		</table>
		...
		<script type="text/javascript">
			...
			$("#btngotoorder").click(function(e){
				...
				$.ajax({
				    ...
				    success:function(data) {
				       location.href="./admin/salesresult.jsp";
			...
			$(".pronumber span.right").click(function(){
				...
				$.ajax({
				    url:'uiupdatesalescarcount', //要请求的url地址
				    type:'POST', //请求方法 GET or POST
				    async:true, //是否使用异步请求的方式
				    timeout:5000, //请求超时的时间，以毫秒计
				    data:{
